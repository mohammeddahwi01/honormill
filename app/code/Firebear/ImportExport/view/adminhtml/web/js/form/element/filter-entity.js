        ...
        return Acstract.extend(general).extend(
            {
                ...
                changeSource: function (value) {
                    ...
                    var types = ['catalog_product', 'advanced_pricing', 'customer', 'customer_address'];

                    var type = entity.value();
                    if (_.indexOf(types, entity.value()) != -1) {
                        type = 'attr';
                    }
                    _.each(this.sourceOptions[type], function (index) {
                        if (index.field == value) {
                            finded = 1;
                            self.changeTypes(index.type);
                            if (index.type == 'select') {
                                self.setOptions(index.select);
                            } else {
                                self.setOptions([]);
                            }
                        }
