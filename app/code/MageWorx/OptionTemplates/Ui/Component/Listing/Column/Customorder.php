...
class Customorder extends \Magento\Ui\Component\Listing\Columns\Column
{
    /**
     * URL builder
     *
    ...
    public function prepareDataSource(array $dataSource)
    {
        ...
        foreach ($dataSource['data']['items'] as &$item) {
            if (!isset($item['custom_order'])) {
                continue;
            }
            
            $found = false;
            $productId = intval($item['entity_id']);
            $fastShip = intval($item['custom_order']);
            ...
            if(!empty($result) && count($result) && count($result1) <= 1) {
                $sql = "SELECT * FROM catalog_product_option_type_value WHERE is_customtab=1 AND option_id=(
            ...
            if (count($result1) > 1 && $found == false) {
                $item['custom_order'] = 'N/A';
            } else if($found) {
                $item['custom_order'] = 'Yes';
            } else {
                $item['custom_order'] = 'No';
            }
        }
        return $dataSource;
    }
