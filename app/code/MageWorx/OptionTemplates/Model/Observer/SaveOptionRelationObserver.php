...
class SaveOptionRelationObserver implements \Magento\Framework\Event\ObserverInterface
{
    ...
    public function execute(\Magento\Framework\Event\Observer $observer)
    {
        ...
        $checkGroupIds = [];
        foreach ($product->getOptions() as $optD) {
            $optData = $optD->getData();
            if (array_key_exists('group_option_id', $optData)) {
                $groupOptionId = $optData['group_option_id'];
                $sql = $connection->select()
                    ->from($tableName)      
                    ->where('option_id = ?', $groupOptionId);
                $result = $connection->fetchAll($sql); 
                if (!empty($result)) {
                    $checkGroupIds[] = $result[0]['group_id'];
                }
            }
        }        
        $this->delGroupSel($checkGroupIds,$product->getId());
        ...
        if ($realDelGroupIds) {
            foreach ($realDelGroupIds as $groupId) {
                $this->removeOption($productId, $groupId);
    ...
    public function delGroupSel($curGroupIds, $productId)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $collection = $objectManager->get('MageWorx\OptionTemplates\Model\ResourceModel\Group\CollectionFactory')->create();
        $values = $productId ? $collection->addProductFilter($productId)->getAllIds() : [];
        $deletedGroupIds = array_diff($values, $curGroupIds);
        $group = $this->groupFactory->create();
        $resource = $group->getResource();
        if ($deletedGroupIds) {
            foreach ($deletedGroupIds as $groupId) {
                $resource->deleteProductRelation($groupId, $productId);
            }
        }
        return true;
    ...
    public function removeOption($productId, $groupId)
    {
        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
        $resource = $objectManager->get('Magento\Framework\App\ResourceConnection');
        $connection = $resource->getConnection();
        $groupOptionIds = [];
        $optionIds = [];
        $groupOptSql = "SELECT * FROM `mageworx_optiontemplates_group_option` WHERE `group_id` = '".$groupId."'";
        $result = $connection->fetchAll($groupOptSql); 
        if (count($result) > 0) {
            $groupOptionIds = array_column($result, 'option_id');
        }
        if (count($groupOptionIds) > 0) {
            $optionSql = "SELECT `option_id` FROM `catalog_product_option` WHERE `product_id` = '".$productId."' AND `group_option_id` IN (".implode(',', $groupOptionIds).")";
            $optionIds = $connection->fetchAll($optionSql);
            if (count($optionIds) > 0) {
                $optionIds = array_column($optionIds, 'option_id');
                $dataSaver = $objectManager->get('MageWorx\OptionBase\Model\ResourceModel\DataSaver');
                $condition = 'option_id IN ('. implode(',', $optionIds) .')';
                $dataSaver->deleteData('catalog_product_option', $condition);
            }
        }
    }
