...
class Value extends \Magento\Catalog\Model\ResourceModel\Product\Option\Value
{
     ...
     * @return void
     */
    public function collectValuesBeforeInsert(ProductCustomOptionInterface &$option, &$optionData, &$currentIncrementIds)
    {
        ...
        foreach ($option->getData('values') as $value) {
            ...
            $updatedValues[] = $this->collectValueData($option, $value, $optionData, $currentIncrementIds);
        }
    ...
    protected function collectValueData($option, $value, &$optionData, &$currentIncrementIds)
    {
        ...
        if (empty($value['mageworx_option_type_id'])) {
            $value['mageworx_option_type_id'] = $this->baseHelper->generateUUIDv4();
        }
