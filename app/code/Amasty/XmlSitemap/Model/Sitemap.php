...
class Sitemap extends AbstractModel implements SitemapInterface
{
                    ...
                    foreach ($images as $image) {
                        ...
                        if ($this->getProductsCaptions()) {
                            ...
                            if (!empty($title)) {
                                ...
                                $xmlLine .= '<image:caption>%s</image:caption>';
                                $xmlParams[] = $title;
                                $xmlParams[] = $title;
                            }
                        }
                        $xmlLine .= '<image:loc>%s</image:loc></image:image>';
                        $xmlParams[] = htmlspecialchars($imageUrl);
                    }
                }
            }
    ...
    private function generateExtra()
    {
        ...
        foreach ($collection as $item) {
            $this->xml[] = sprintf(
                '<url><loc>%s</loc><changefreq>%s</changefreq><priority>%.2f</priority></url>',
    ...
    private function generateNavigation()
    {
        ...
        foreach ($collection as $item) {
            $url = $this->_getStoreBaseUrl() . $item->getUrl();
    ...
    private function _writeIndexFile($pieces)
    {
        ...
        $stream->write('<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9" xmlns:xhtml="http://www.w3.org/1999/xhtml">');
        foreach ($pieces as $url) {
            $item = sprintf(
                '<sitemap><loc>%s</loc><lastmod>%s</lastmod></sitemap>',
                htmlspecialchars($this->helper->getCorrectUrl($url, $this->getStoreId())),
                $this->date
            );
            $stream->write($item . "\n");
        }
    ...
    private function _isUrlToExclude($url)
    {
        ...
        if ($this->excludeUrls != '' && strpos($this->excludeUrls, $url) !== false) {
            $isToExclude = true;
        }
    ...
    {
        return $this->_getData(SitemapInterface::EXCLUDE_URLS);
    }
