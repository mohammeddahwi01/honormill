...
class GoogleWizard
{
    ...
    {
        $config = [
            RegistryContainer::VAR_STEP => ++$requestData['step']
        ];
    ...
    {
        $this->checkAndPasteVars(
            $requestData, [
                'store_id', 'format_price_currency',
                'delivery_host', 'delivery_user', 'delivery_password',
                'delivery_passive_mode', 'execute_mode',
                'delivery_type', 'filename', 'cron_day', 'cron_time'
            ]
        );
        ...
        if (empty($ftpHost)) {
            $this->feed->setData(
                'delivery_type', \Amasty\Feed\Model\Feed::DELIVERY_TYPE_DLD
            );
        }
    ...
    protected function loadAttribute($element)
    {
        $elementModel = $this->googleWizardElementFactory->create(
            [
                'elementType' => $element, 'feed' => $this->feed
            ]
        );
        ...
        if (is_array($sessionData)) {
            ...
            if (isset($sessionData['basic'])) {
                $attributesData = array_merge(
                    $attributesData, $sessionData['basic']
                );
            }

            if (isset($sessionData['optional'])) {
                $attributesData = array_merge(
                    $attributesData, $sessionData['optional']
                );
            }
