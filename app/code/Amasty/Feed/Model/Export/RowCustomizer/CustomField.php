...
class CustomField implements RowCustomizerInterface
{
    ...
    public function prepareData($collection, $productIds)
    {
        if ($this->export->hasAttributes(Export::PREFIX_CUSTOM_FIELD_ATTRIBUTE)) {
            ...
            if (!empty($data)) {
                ...
                $nonExistentAttributes = implode(",", $attributes);
                throw new NoSuchEntityException(__('Error(s) occurred during feed generation, attribute code(s): "%1"', $nonExistentAttributes));
            }
        }
    }
    ...
    {
        $modifierArray = preg_split('/([\d]+([.,][\d]+)?)/', $modifier, -1, PREG_SPLIT_DELIM_CAPTURE | PREG_SPLIT_NO_EMPTY);
